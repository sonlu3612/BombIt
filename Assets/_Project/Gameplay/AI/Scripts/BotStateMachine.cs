...
namespace _Project.Gameplay.AI.Scripts
{
    ...
    public static class BotRuntimeDebugLog
    {
        ...
        public static void LogMapSnapshot(MapContext mapContext)
        {
            ...
            lock (Sync)
            {
                if (!DumpedMapContexts.Add(mapContext.GetInstanceID()))
                    return;
            }
            ...
            List<Vector3Int> liveBlockCells = mapContext.MapBuilder != null ? mapContext.MapBuilder.GetActiveBlockCells() : new List<Vector3Int>();
        ...
        public static void LogBotThink(PlayerController player, BotSenseContext sense, BotBlackboard blackboard, string stateName)
        {
            ...
            WriteRaw(
                "BOT",
                $"{DescribePlayer(player)} THINK state={stateName} cell={FormatCell(sense.CurrentCell)} target={target} escape={escape} path={pathIndex}/{pathCount} danger={sense.DangerCells.Count} blocked={sense.BlockedCells.Count} items={sense.ItemCells.Count} enemies={sense.EnemyCells.Count} breakable={sense.BreakableBlocks.Count}");
        }
        ...
        {
            EnsureSession();
            string bombName = bomb != null ? $"{bomb.name}#{bomb.GetInstanceID()}" : "Bomb";
        ...
        {
            return $"{player.name}#{player.GetInstanceID()}";
        }
